...
#          )
 ...
 #         wardId=LETTERS[1:nrow(plan)]
        ...
        datCoord<-do.call("rbind", 
                          replicate(input$nBed, datCoord, simplify=FALSE))
        ...
        png(filename=paste0(plotFolder,plName), 
            width=aspPr*500, 
            height=500, units="px")  
        print(
          ggplot(pol, aes(x=x, y=y)) + 
        ...
        dat1<-
          dat1 %>%
          mutate(wardN=ifelse(wardN>input$nBed, NA, wardN))
